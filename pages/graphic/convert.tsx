...
import { Select, Button, FileDrop } from '@/components/inputs'
import { ToolTip, StatusGood, StatusBad, StatusLoading } from '@/components/utility'

import Icon from '@mdi/react'
import { mdiFileMoveOutline, mdiClose, mdiDownload, mdiTrashCanOutline } from '@mdi/js'
	...
	return (
		...
		>
			...
			{failed ? null : done ? (
				...
				<Button style='flat' hint='Cancel' icon={mdiClose} onClick={removeSelf} />
			)}
			{done || failed ? (
				<Button style='flat' hint='Delete' icon={mdiTrashCanOutline} onClick={removeSelf} />
			) : null}
		</div>
	)
}
	...
	return (
		<Page title='Image Converter'>
				...
				items={[
					{
						...
						control: (
							<Select
								...
								options={validFormats.map((format, i) => ({
									key: i.toString(),
									value: format.name,
								}))}
								onChange={(e: Event) =>
									setOutFormat(parseInt((e.target as HTMLSelectElement).value))
								}
							/>
						),
					},
			...
			<Segment
				...
				body={jobs.map((job, i) => (
					<Job key={i} removeSelf={() => setJobs(jobs.splice(i, 1))} {...job} />
				))}
			/>
		</Page>
	)
