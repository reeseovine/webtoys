...
const Tool = () => {
	...
	return (
		<Page title='Text Inspector &amp; Case Converter'>
			<Segment
				title='Convert case'
				body={
			...
			>
				...
				<div className='grow md:basis-1/3'>
					...
					<div className='grid grid-cols-2'>
						...
						<div>Paragraphs:</div>{' '}
						<div>{prettyDec(input.split(/\n/).filter((l) => l.trim().length > 0).length)}</div>
					</div>

					<H2 className='mt-3'>Word distribution</H2>
					<TextArea value={wordDistribStr} disabled={true} rows={10} />

					<H2 className='mt-3'>Character distribution</H2>
					<TextArea value={charDistribStr} disabled={true} rows={10} />
				</div>
			</div>
		</Page>
	)
