...
import { Button, Select, Number, TextField, TextArea } from '@/components/inputs'

import Icon from '@mdi/react'
import { mdiTuneVariant, mdiNumeric, mdiWeb, mdiAccount, mdiSync } from '@mdi/js'
	...
	return (
		<Page title='UUID Generator'>
			...
			<Segment
				...
				controls={[
					version === '1' || version === '4' ? (
						<Button
							icon={mdiSync}
							hint='Regenerate'
							onClick={() => setNonce(new Date().getTime())}
						/>
					) : null,
				...
				body={
					version === '3' || version === '5' ? (
						<TextField value={output} disabled={true} />
					) : (
						<TextArea value={output} disabled={true} rows={count} />
					)
				}
			/>
		</Page>
