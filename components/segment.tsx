...
import { Button, FileButton } from '@/components/inputs'
		...
		{items.map((item) =>
			item === null ? null : (
				<div
					...
					'
				>
					<Icon path={item.icon} size={1} className='hidden sm:block shrink-0' />
					<div className='grow'>
				...
				case 'clear':
					return (
						<Button
							key={i}
							icon={mdiClose}
							hint={control.hint ? control.hint : 'Clear'}
				...
				case 'copy':
					return (
						<Clipboard key={i} data-clipboard-text={control.data}>
							<Button
								icon={mdiContentCopy}
								hint={control.hint ? control.hint : 'Copy'}
								showSuccess={true}
							/>
						</Clipboard>
					)
				case 'file':
					return <FileButton key={i} cb={control.callback} />
				default:
					return control
			}
		})
